...
used_sents = []
cur_sent_idx = 0
for i, bookcorpus_path in enumerate(config.bookcorpus_paths):
    log.infov('loading bookcorpus [{}/{}]: {}'.format(
        i, len(config.bookcorpus_paths), bookcorpus_path))

    sent_list = open(bookcorpus_path, 'r').read().splitlines()
...
save_path = os.path.join(config.bookcorpus_dir, 'merge_and_count.pkl')
log.warn('saving results to : {}'.format(save_path))
cPickle.dump({
    'vocab_1st2sent_idx_list': vocab_1st2sent_idx_list,
    'most_common': most_common,
}, open(save_path, 'wb'))
save_text_path = os.path.join(config.bookcorpus_dir, 'merged_used_sents.txt')
