...
class Model(object):
    ...
    def build(self):
        ...
        q_L_ft = modules.encode_L(q_embed, self.batch['q_intseq_len'], L_DIM,
                                  cell_type='GRU')
        q_L_ft2 = q_linear_v = modules.fc_layer(
            q_L_ft, V_DIM, use_bias=True, use_bn=False, use_ln=True,
            activation_fn=tf.tanh, is_training=self.is_train,
            scope='q_L_ft2')
        self.heavy_output['condition'] = q_L_ft2
        ...
        l_linear_l = modules.fc_layer(
            q_L_ft2, L_DIM, use_bias=True, use_bn=False, use_ln=True,
            activation_fn=tf.nn.relu, is_training=self.is_train,
            scope='q_linear_l')
