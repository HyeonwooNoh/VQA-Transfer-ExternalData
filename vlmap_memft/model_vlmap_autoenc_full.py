...
class Model(object):
    ...
    def build(self):
        ...
        # hist summary
        for key, val in self.vis_hist.items():
            tf.summary.histogram('train-{}'.format(key), val,
                                 collections=['heavy_train'])
            tf.summary.histogram('val-{}'.format(key), val,
                                 collections=['heavy_val'])
    ...
    def build_object_predict(self):
        ...
        label_embed = tf.nn.embedding_lookup(  # [bs, #obj, W_DIM]
            self.l_answer_word_map, self.batch['obj_pred/labels'])
    ...
    def build_attribute_predict(self):
        ...
        attr_vec = modules.fc_layer(
            obj_attr_embed, L_DIM, use_bias=True, use_bn=False, use_ln=False,
            activation_fn=None, is_training=self.is_train,
        ...
        l_linear_l = modules.fc_layer(
            attr_vec_noise, L_DIM, use_bias=True, use_bn=False, use_ln=True,
            activation_fn=tf.nn.relu, is_training=self.is_train,
            scope='q_linear_l')
    ...
    def build_object_blank_fill(self):
        ...
        fill_vec = modules.fc_layer(
            blank_fill_ft, L_DIM, use_bias=True, use_bn=False, use_ln=False,
            activation_fn=None, is_training=self.is_train,
        ...
        l_linear_l = modules.fc_layer(
            fill_vec_noise, L_DIM, use_bias=True, use_bn=False, use_ln=True,
            activation_fn=tf.nn.relu, is_training=self.is_train,
            scope='q_linear_l')
    ...
    def build_attribute_blank_fill(self):
        ...
        fill_embed = tf.nn.embedding_lookup(self.l_answer_word_map,
                                            self.batch['attr_blank_fill/fills'])
        fill_embed2 = modules.fc_layer(
            fill_embed, L_DIM, use_bias=True, use_bn=False, use_ln=True,
            activation_fn=tf.nn.tanh, is_training=self.is_train,
