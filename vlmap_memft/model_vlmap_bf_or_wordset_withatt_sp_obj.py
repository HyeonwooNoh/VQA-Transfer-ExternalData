...
class Model(object):
    ...
    def build(self):
        ...
        self.build_object_bf_V_ft()
        self.build_attribute_bf_V_ft()
    ...
    def build_object_predict(self):
        ...
        V_ft = tf.tile(V_ft, [1, self.data_cfg.n_obj_pred, 1, 1])  # [bs, #obj, #proposal, #feat_dim]
        V_ft = tf.reshape(
        ...
        num_V_ft = tf.tile(num_V_ft, [1, self.data_cfg.n_obj_pred])  # [bs, #obj]
        ...
        pooled_V_ft = tf.reshape(
            flat_pooled_V_ft, [-1, self.data_cfg.n_obj_pred, self.data_cfg.vfeat_dim])
    ...
    def build_attribute_bf_V_ft(self):
        V_ft = self.batch['image_ft']  # [bs,  #proposal, #feat_dim]
        V_ft = tf.expand_dims(V_ft, axis=1)  # [bs, 1, #proposal, #feat_dim]
