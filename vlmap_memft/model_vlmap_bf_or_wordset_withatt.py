...
class Model(object):
    ...
    def build_object_V_ft(self):
        ...
        self.mid_result['object_V_ft'] = V_ft
        self.mid_result['object_num_V_ft'] = num_V_ft
    ...
    def build_object_wordset(self):
        ...
        q_linear_v = modules.fc_layer(  # [bs, #obj, V_DIM]
            wordset_ft, V_DIM, use_bias=True, use_bn=False, use_ln=True,
            activation_fn=tf.nn.relu, is_training=self.is_train,
        ...
        att_score = modules.hadamard_attention(  # [bs * #obj, len]
            ...
            use_ln=False, is_train=self.is_train, scope='wordset_att')
    ...
    def build_attribute_V_ft(self):
        ...
        self.mid_result['attribute_V_ft'] = V_ft
        self.mid_result['attribute_num_V_ft'] = num_V_ft
    ...
    def build_attribute_wordset(self):
        ...
        q_linear_v = modules.fc_layer(  # [bs, #attr, V_DIM]
            ...
            activation_fn=tf.nn.relu, is_training=self.is_train,
            scope='wordset_q_linear_v')
        flat_q_linear_v = tf.reshape(q_linear_v, [-1, V_DIM])  # [bs * #attr, V_DIM]

        att_score = modules.hadamard_attention(  # [bs * #attr, len]
            v_linear_v, num_V_ft, flat_q_linear_v,
            use_ln=False, is_train=self.is_train, scope='wordset_att')
        flat_pooled_V_ft = modules.attention_pooling(V_ft, att_score)  # [bs * #attr, V_DIM]
        pooled_V_ft = tf.reshape(
    ...
    def build_object_blank_fill(self):
        ...
        q_linear_v = modules.fc_layer(  # [bs, #obj, V_DIM]
            blank_ft, V_DIM, use_bias=True, use_bn=False, use_ln=True,
            activation_fn=tf.nn.relu, is_training=self.is_train,
            scope='bf_q_linear_v')
        flat_q_linear_v = tf.reshape(q_linear_v, [-1, V_DIM])  # [bs * #obj, V_DIM]
        ...
        att_score = modules.hadamard_attention(  # [bs * #obj, len]
            v_linear_v, num_V_ft, flat_q_linear_v,
            use_ln=False, is_train=self.is_train, scope='bf_att')
        flat_pooled_V_ft = modules.attention_pooling(V_ft, att_score)  # [bs * #obj, vfeat_dim]
        pooled_V_ft = tf.reshape(
            flat_pooled_V_ft, [-1, self.data_cfg.n_obj_bf, self.data_cfg.vfeat_dim])
