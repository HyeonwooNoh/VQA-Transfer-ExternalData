...
class Model(object):
    ...
    def build_object_enwiki(self):
        ...
        flat_enwiki_ft = modules.encode_L(  # [bs * #proposal, L_DIM]
            ...
            tf.reshape(enwiki_len, [-1]), L_DIM,
            scope='encode_L_enwiki', cell_type='GRU')
        enwiki_ft = tf.reshape(
            flat_enwiki_ft, [-1, self.data_cfg.n_obj_bf, L_DIM])
        ...
        l_linear_l = modules.fc_layer(
            enwiki_ft, L_DIM, use_bias=True, use_bn=False, use_ln=True,
            activation_fn=tf.nn.relu, is_training=self.is_train,
            scope='q_linear_l')
        ...
        logit = modules.fc_layer(
            ...
            use_bias=True, use_bn=False, use_ln=False,
            activation_fn=None, is_training=self.is_train, scope='classifier')
        self.mid_result['obj_enwiki/logit'] = logit  # [bs, #obj, #answer]
    ...
    def build_attribute_enwiki(self):
        ...
        flat_enwiki_ft = modules.encode_L(  # [bs * #proposal, L_DIM]
            ...
            scope='encode_L_enwiki', cell_type='GRU')
        enwiki_ft = tf.reshape(
            flat_enwiki_ft, [-1, self.data_cfg.n_attr_bf, L_DIM])
        ...
        logit = modules.fc_layer(
            ...
            use_bias=True, use_bn=False, use_ln=False,
            activation_fn=None, is_training=self.is_train, scope='classifier')
        self.mid_result['attr_enwiki/logit'] = logit  # [bs, #attr, #answer]
